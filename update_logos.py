...
# Config
# -----------------------
DEFAULT_SCHEDULES_DIR = "schedule"  
DEFAULT_OUT_DIR = "downloaded-images"
DEFAULT_BASE_URL = "https://cdn.tv-programma.it/wp-content/uploads"
...
# -----------------------
def process_json_file(json_path: Path, out_root: Path, base_url: str, workers: int):
    ...
    # Channel folder is based on the file name (e.g., "20.json" -> "20")
    channel_folder = json_path.stem
    target_dir = out_root / channel_folder
...
def main():
    ...
    if not schedules_dir.exists():
        logger.error(f"Schedules directory '{schedules_dir}' does not exist.")
        return

    # Process all JSON files directly inside the schedule directory
    for json_path in sorted(schedules_dir.glob("*.json")):
        process_json_file(json_path, out_root, base_url, workers)
